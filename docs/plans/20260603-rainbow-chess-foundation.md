...
- [x] create multi-stage `Dockerfile` adapted from virusgame (build Go server in golang-alpine, copy binary + static frontend into alpine, inject `COMMIT_SHA`, `EXPOSE 8080`) — **no bot-hoster stage**
- [x] create `docker-compose.yml` adapted from virusgame: Traefik labels (host rule, websecure, certresolver, service port 8080, WS upgrade headers), SQLite volume (`./backend/data:/app/backend/data`), GHCR image with `${GIT_SHA}`, `restart: unless-stopped`
- [x] add `.env.example` documenting `HOSTNAME`, `NETWORK_NAME`, `GIT_SHA`, and the GHCR image owner/repo
- [x] verify `docker build .` succeeds and the container serves the app on `:8080` (build + smoke as the automatable check)
- [x] run `go test ./...` - must pass before next task
